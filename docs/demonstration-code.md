 ...
 * * CONTROL LOGIC:
 * - When VPD is too high and leaf is dry → Activate mist sprayer (MQTT)
 * - When VPD is too high but leaf is wet → Do nothing (leaf already moist - risking leaf rot)
 * - when VPD is too LOW → Activate fan(only if outdoor air is dry AND temp > 18°C)
 * - VPD optimal → All off
 ********************/
...
// VPD measured in kPa - optimal range is set to: 0.4 - 0.65 kPa - for colder weather
const float VPD_LOW_THRESHOLD  = 0.4; // Below this → Fan ON (increase VPD)
const float VPD_HIGH_THRESHOLD = 0.65;   // Above this → Mist ON (decrease VPD)
...
// ==================== Mist Pulse State ====================
bool mistPulseActive = false;     // Checking if pulse cycle is on
bool mistPulseOn     = false;     // checking if mist is on
...
void runControlLogic() {
  ...
  if (vpdIndoor < VPD_LOW_THRESHOLD) {
    if (humOutdoor < 60 && tempIndoor > 18.0) {
      ...
      Serial.println("   → VPD LOW: Fan ON (outdoor air dry & temp OK)");
    } 
    else {
      shouldFanBeOn = false;
      shouldMistBeOn = false;
      Serial.println("   → VPD LOW: Fan OFF (outdoor air not dry OR temp too low)");
    }
  }
 ...
 if (pulseMode) {
  // mist is controlled by handleMistPulse()
  shouldMistBeOn = false;
}
  ...
  if (!pulseMode && mistState) {
    if (leafSensorRaw > (LEAF_WET_THRESHOLD + HYSTERESIS_BUFFER)) {
      ...
      shouldMistBeOn = false;
      Serial.println("   → Hysteresis: Turning mist OFF (VPD normal)");
    } else {
      shouldMistBeOn = true;
      Serial.println("   → Hysteresis: Keeping mist ON");
    }
  }
...
void handleRoot() {
  ...
  page += String("<div class='card card-blue' style='background-color:")
        + vpdOutColor + ";'><div class='icon'>🌡</div>";
  ...
  // --- Fan & Mist Card ---
String fanIcon  = fanState  ? "🌬" : "";
String mistIcon = mistState ? "💦" : "";
...
page += String("<div class='value'>Mist: <span class='")
  ...
  page += String("<div class='card' style='background-color:")
        + indoorPlantColor + ";'><div class='icon'>🌿</div>";
  ...
  page += String("<div class='card' style='background-color:")
        + outdoorPlantColor + ";'><div class='icon'>🌳</div>";
...
// This function calculates a plant health score (0–100) based on the current VPD value. The closer the VPD is to the optimal range, the higher the score.
